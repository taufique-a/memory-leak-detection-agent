...
Every page whose trend was not clearly flat - GROWING, or INCONCLUSIVE because the readings were too noisy to call - is repeated between two heap snapshots (through Chrome DevTools MCP when it is available, otherwise the raw DevTools protocol; the report and the page say which). What the two snapshots give, per page: the whole-heap shallow total before and after, the object count, detached DOM nodes before and after, console problems and failed requests during the run, and the object types that gained the most with their shallow and retained size change. Per finding: instances left behind, shallow size, retained size, and a **severity** (HIGH from 5 MB retained per journey, MEDIUM from 512 KB or any PROVEN leak, LOW otherwise) shown beside, never merged with, confidence. The cause is **read off the real retaining path**: `DOMTimer`/`ScheduledAction` → a timer; `*EventListener` → a listener; Mutation/Resize/Intersection/PerformanceObserver → an observer; `Subscriber`/`Subject` → a subscription; `WebSocket`/`EventSource`, `Worker`, an animation-frame callback, a property on `window`, detached DOM, a closure. If no step on the path names a mechanism, the answer is **"undetermined"** - never a plausible default.
...
### How long it takes, and why (`timings` on every page result)

Measured on a small React app on a quiet machine: a page that stays flat costs one trend run (about 17 s for 8 visits plus warm-up); a leaking page costs the trend run (11 s), the confirmation run when growth is modest (20 s), and the two heap snapshots with their retained-size analysis (23 s here; minutes on an application the size of IOSense, where the retained-size computation over hundreds of thousands of objects dominates). Those figures are recorded per page and shown under technical details and in the report, so a slow check says where the time went. The same run under memory pressure (under 5 GB free, another dev server building) took ten times longer, and once died during the snapshots: the machine, not the method. What is deliberately not traded for speed: the visits per page, the confirmation of modest growth, and the second snapshot. Pages are measured one after another because forced garbage collection assumes it has the browser to itself.
