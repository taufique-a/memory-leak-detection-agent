...
| 6 | Chrome DevTools memory analysis | DONE | Forced-GC trend per journey; two heap snapshots for every GROWING or INCONCLUSIVE page (Chrome DevTools MCP when available, else the raw protocol - the page says which); per page: heap total before/after, object count, detached DOM before/after, console problems; per type: instances, shallow, retained; per finding: severity beside confidence; modest growth re-measured before it is believed | `dominators`, `checkEndToEnd`, `checkPlainJs` |
| 7 | Evidence-based leak detection | DONE | Severity (cost) and six confidence levels (certainty) shown side by side, in plain words: Confirmed leak (PROVEN), Strong evidence (HIGH), Possible leak (MEDIUM/LOW/UNKNOWN), Inconclusive. Never a suspicion as a confirmed leak; browser-internal objects excluded and named | `checkUnits`, `checkEndToEnd` |
...
Per page on a small app, quiet machine: flat page 17 s; leaking page 11 s trend + 20 s confirmation + 23 s snapshots (Chrome DevTools MCP 1.9.0). Recorded per page (`timings`), shown under technical details and in the report. Under memory pressure the same run was ten times slower - keep a few GB free and stop other builds while a check runs. Not traded for speed: visits per page, confirmation of modest growth, the second snapshot, sequential measurement.
