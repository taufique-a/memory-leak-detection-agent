...
export function renderHtml(inv: Investigation): string {
  ...
  if (inv.scenario.gathered) {
    ...
    p('<h3>Scenario</h3>');
    p(`<p><strong>${esc(s.name)}</strong> &mdash; ${esc(s.description)}</p>`);
    p(`<p>Repeated <strong>${s.iterations}</strong> times. Each iteration:</p><ol>`);
    for (const step of s.steps) p(`<li>${esc(step)}</li>`);
    p('</ol>');
  } else {
  ...
  if (inv.reproductionSteps.gathered) {
    p('<h3>Reproduction steps</h3>');
    p('<p class="meta">Follow these by hand to see the same behaviour without the agent.</p><ul>');
    for (const step of inv.reproductionSteps.data) p(`<li>${esc(step)}</li>`);
    p('</ul>');
  } else {
  ...
  if (inv.runtimeFindings.gathered) {
    p(renderRuntimeHtml(inv.runtimeFindings.data));
  } else {
    p(renderPlaceholder(inv.runtimeFindings, 'Runtime findings'));
  }

  if (inv.memoryEvidence.gathered) {
    p(renderMemoryHtml(inv.memoryEvidence.data));
  } else {
...
function renderRuntimeHtml(r: RuntimeObservations): string {
  const parts: string[] = ['<h3>Runtime findings</h3>'];

  parts.push(`<ul>
    <li>Chrome ${esc(r.chromeVersion)}</li>
    <li>Iterations completed: <strong>${r.iterationsCompleted} / ${r.iterationsRequested}</strong></li>
    <li>Step failures: <strong>${r.stepFailures.length}</strong></li>
  </ul>`);

  if (r.abortedReason !== undefined) {
    parts.push(
      `<div class="notice warn"><p><strong>Run aborted.</strong> ${esc(r.abortedReason)}</p></div>`,
    );
  }

  if (r.stepFailures.length > 0) {
    parts.push(`<div class="notice warn"><p>Some steps failed, so the journey performed was
      not the one written down. Read the memory numbers with that in mind.</p></div>`);
    parts.push('<div class="table-wrap"><table><thead><tr><th>Iteration</th><th>Step</th><th>Error</th></tr></thead><tbody>');
    for (const f of r.stepFailures.slice(0, 10)) {
      parts.push(
        `<tr><td>${f.iteration}</td><td>${esc(f.description)}</td><td>${esc(f.error)}</td></tr>`,
      );
    }
  ...
  if (r.console.length > 0) {
    parts.push('<p><strong>Console errors during the run</strong></p>');
    parts.push('<div class="table-wrap"><table><thead><tr><th>Count</th><th>Type</th><th>Message</th></tr></thead><tbody>');
    for (const entry of r.console) {
      const text = entry.text.length > 200 ? entry.text.slice(0, 199) + '…' : entry.text;
      parts.push(
        `<tr><td class="points">${entry.count}</td><td>${esc(entry.type)}</td><td><code>${esc(text)}</code></td></tr>`,
      );
    }
    parts.push('</tbody></table></div>');
    parts.push(`<p class="meta">An error appearing exactly once per iteration is worth
      attention: it means something fails on every pass of the journey, which often points
      at state left behind by the previous pass.</p>`);
  } else {
    parts.push('<p>No console errors were recorded.</p>');
  }

  return parts.join('\n');
}

function renderMemoryHtml(m: MemoryEvidence): string {
  const parts: string[] = ['<h3>Memory evidence</h3>'];

  const badge =
    m.verdict === 'GROWING' ? 'CRITICAL' : m.verdict === 'STABLE' ? 'LOW' : 'MEDIUM';
  parts.push(`<p><span class="badge ${badge}">${esc(m.verdict)}</span></p>`);

  parts.push('<div class="table-wrap"><table><tbody>');
  const rows: Array<[string, string]> = [
    ['Growth per iteration', signedMb(m.bytesPerIteration)],
    ['Total change', signedMb(m.totalDeltaBytes)],
    ['Line fit (R²)', m.rSquared.toFixed(3)],
    ['Attached DOM per iteration', m.attachedDomPerIteration.toFixed(1)],
    ['Listeners per iteration', m.listenersPerIteration.toFixed(2)],
    ['Samples analysed', String(m.samplesAnalysed)],
    ['Warm-up discarded', String(m.warmupDiscarded)],
  ];
  for (const [k, v] of rows) parts.push(`<tr><th>${esc(k)}</th><td><strong>${esc(v)}</strong></td></tr>`);
  parts.push('</tbody></table></div>');

  parts.push(`<p>${esc(m.interpretation)}</p>`);

  if (m.caveats.length > 0) {
    parts.push('<p><strong>Caveats</strong></p><ul>');
    for (const caveat of m.caveats) parts.push(`<li>${esc(caveat)}</li>`);
    parts.push('</ul>');
  }
  ...
  parts.push(
    ungced === 0
      ? `<p class="meta">Every reading was taken after a forced garbage collection, so these
         are bytes that survived collection rather than uncollected garbage.</p>`
      : `<div class="notice warn"><p><strong>${ungced} reading(s) were taken without a forced
         collection</strong> and may include uncollected garbage. Treat the trend as
         indicative only.</p></div>`,
  );

  /* ---- inline bar chart, no external library ---- */
  const values = m.measurements.map((x) => x.jsHeapUsedBytes ?? 0);
  const min = Math.min(...values);
  const max = Math.max(...values);
  const span = max - min || 1;

  parts.push('<p><strong>Readings</strong></p>');
  parts.push(
    `<p class="meta">Scale ${esc(mb(min))} &ndash; ${esc(mb(max))}, range ${esc(mb(max - min))}.` +
      (max - min < 512 * 1024
        ? ' The range is tiny, so the bars below are magnified &mdash; this line is flat.'
        : '') +
      '</p>',
  );
  parts.push('<div class="table-wrap"><table><thead><tr>');
  parts.push('<th>Iteration</th><th>Heap</th><th>Attached DOM</th><th>Listeners</th><th></th>');
  parts.push('</tr></thead><tbody>');
  for (const x of m.measurements) {
    const heap = x.jsHeapUsedBytes ?? 0;
    const pct = Math.round(((heap - min) / span) * 100);
    parts.push(
      `<tr><td>${x.iteration}</td><td class="points">${esc(mb(heap))}</td>` +
        `<td class="points">${x.attachedDomNodes ?? '-'}</td>` +
        `<td class="points">${x.listeners ?? '-'}</td>` +
        `<td style="width:40%"><div style="background:var(--accent);height:.7em;border-radius:2px;width:${Math.max(1, pct)}%"></div></td></tr>`,
    );
  }
  parts.push('</tbody></table></div>');
