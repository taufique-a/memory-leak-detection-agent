...
Verified by building a throwaway git repo with a real Angular component and
running the whole apply path over it. It works, and its limits are narrow on
purpose.

**It writes exactly one kind of change.** `takeUntil(this.destroy$)` where
`destroy# Memory Leak Agent — Runbook
...
---
 ...
 is never completed, and only when the component **already has an
`ngOnDestroy`**. Two statements are appended to that method; no existing line
is touched. The developer already wrote the intent — the only thing missing is
firing the signal.

Everything else is **described, not written**: an empty `ngOnDestroy`, a
missing one, a handle never released. Those need a judgement call about what
belongs where, and this tool does not make judgement calls in your source.

**On IOSense today it would write nothing.** Measured against the current HEAD:

| | Count |
|---|---|
| Findings | 2,590 |
| With `DESTROY_SUBJECT_NEVER_COMPLETED` | 30 |
| …with the signal identified | 11 |
| …**and** an existing `ngOnDestroy` | **0** |

Every broken-`destroy# Memory Leak Agent — Runbook
...
---
 ...
 component in IOSense lacks `ngOnDestroy` entirely, so
the fixer stops and says so rather than adding the method, the
`implements OnDestroy`, and the import unattended. That is a real gap, not a
failure — the fix would be mechanical, but it edits the class declaration and
the imports, which is a bigger change than the current safety rules allow.

**What it guarantees when it does write:**

- refuses if you have any uncommitted work, naming the files in the way
- works only on a `memory-agent/<id>` branch, never yours
- records the baseline commit before touching anything
- asks about each change separately — `--yes` is not reachable from the UI
- **commits** on that branch, so the printed rollback commands actually undo it
- runs your own build, lint and tests afterwards
- prints the exact commands to throw it all away
